...
public class World : IBinarySerializable
{
    ...
    {
        string txt = $"World: Tick={Tick}, NextEntityId={NextEntityId}";

        txt += $"Blocks ({Blocks.Count}):";
    ...
    public void Serialize(BinaryStreamHandler writer)
    {
        ...
        foreach (Entity entity in Entities)
        {
            if (entity is not IBinarySerializable serializable)
                throw new InvalidOperationException($"Entity type '{entity.GetType().Name}' is not binary serializable.");
    ...
    public static IBinarySerializable Deserialize(BinaryStreamHandler reader)
    {
        ...
        EntityId nextEntityId = reader.Read<EntityId>();
