...
public static class GameWindow
{
    ...
    private static void UpdateInput(SharedInputState input)
    {
        if (Raylib.IsKeyDown(KeyboardKey.Left))
            input.Press(KeyState.Left);
        else
            input.Release(KeyState.Left);

        if (Raylib.IsKeyDown(KeyboardKey.Right))
            input.Press(KeyState.Right);
        else
            input.Release(KeyState.Right);

        if (Raylib.IsKeyDown(KeyboardKey.Up))
            input.Press(KeyState.Up);
        else
            input.Release(KeyState.Up);

        if (Raylib.IsMouseButtonPressed(MouseButton.Left))
        {
            input.Press(KeyState.MouseLeft);

            Vector2 mouseScreen = Raylib.GetMousePosition();

            input.Write(mouseScreen.X / 20, mouseScreen.Y / 20);
        }
        else
        {
            input.Release(KeyState.MouseLeft);
        }
    }
}
